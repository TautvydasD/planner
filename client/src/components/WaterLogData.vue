...
export default {
  ...
  methods: {
    ...
    removeWaterLog(item) {
      return user.removeWaterLog(item.id).then(() => {
    ...
    createLog(item) {
      ...
      return user.createWaterLog(bodyData).then(({ data }) => {
    ...
    saveEdit(item) {
      ...
      return user.editWaterLog(item.id, bodyData).then(({ data }) => {
